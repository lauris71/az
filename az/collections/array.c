...
array_serialize (const AZImplementation *impl, void *inst, unsigned char *d, unsigned int dlen, AZContext *ctx)
{
	...
	unsigned int len = az_instance_serialize(&AZUint32Klass.impl, &array->collection.element_type, d, dlen, ctx);
	...
	for (unsigned int i = 0; i < array->collection.size; i++) {
		len += az_instance_serialize(AZ_IMPL_FROM_TYPE(array->collection.element_type), az_array_value_at(array, i), d + len, (len <= dlen) ? dlen - len : 0, ctx);
	}
	return len;
}
...
array_deserialize (const AZImplementation *impl, AZValue *value, const unsigned char *s, unsigned int slen, AZContext *ctx)
{
	...
	unsigned int len = 0;
	len += az_value_deserialize(&AZUint32Klass.impl, (AZValue *) &array->collection.element_type, s + len, (len <= slen) ? slen - len : 0, ctx);
	len += az_value_deserialize(&AZUint32Klass.impl, (AZValue *) &array->collection.size, s + len, (len <= slen) ? slen - len : 0, ctx);
	array->values = az_value_new_array(AZ_IMPL_FROM_TYPE(array->collection.element_type), array->collection.size);
	for (unsigned int i = 0; i < array->collection.size; i++) {
		len += az_value_deserialize(AZ_IMPL_FROM_TYPE(array->collection.element_type), az_array_value_at(array, i), s + len, (len <= slen) ? slen - len : 0, ctx);
	}
	return len;
}
...
array_get_element_type(const AZCollectionImplementation *coll_impl, AZCollection *coll_inst)
{
	...
	return array->collection.element_type;
}
...
array_contains(const AZCollectionImplementation *coll_impl, AZCollection *coll_inst, const AZImplementation *impl, const void *inst)
{
	AZArray *array = (AZArray *) coll_inst;
	for (unsigned int i = 0; i < array->collection.size; i++) {
		const AZValue *val = (const AZValue *) ((char *) array->values + i * AZ_CLASS_ELEMENT_SIZE(AZ_CLASS_FROM_TYPE(array->collection.element_type)));
		if (az_value_equals_instance_autobox(AZ_IMPL_FROM_TYPE(array->collection.element_type), val, impl, inst)) return 1;
	}
	return 0;
}
...
array_get_element (const AZListImplementation *list_impl, void *list_inst, unsigned int idx, AZValue *val, unsigned int size)
{
	...
	return az_value_copy_autobox(AZ_IMPL_FROM_TYPE(array->collection.element_type), val, (const AZValue *) ((char *) array->values + idx * AZ_CLASS_ELEMENT_SIZE(AZ_CLASS_FROM_TYPE(array->collection.element_type))), size);
}
...
array_object_instance_finalize(AZArrayObjectClass *klass, AZArrayObject *aobj)
{
	if (!az_object_flags((AZObject *) aobj, AZ_ARRAY_OBJ_FLAG_OWNED)) {
		az_value_delete_array(AZ_IMPL_FROM_TYPE(aobj->array.collection.element_type), aobj->array.values, aobj->array.collection.size);
	}
}
...
array_object_element_type(const AZCollectionImplementation *coll_impl, AZCollection *coll_inst)
{
	...
	return aof->array.collection.element_type;
}
...
{
	AZArrayObject *aof = (AZArrayObject *) coll_inst;
	return aof->array.collection.size;
}
...
array_object_contains(const AZCollectionImplementation *coll_impl, AZCollection *coll_inst, const AZImplementation *impl, const void *inst)
{
	...
	for (unsigned int i = 0; i < aobj->array.collection.size; i++) {
		const AZValue *val = (const AZValue *) ((char *) aobj->array.values + i * AZ_CLASS_ELEMENT_SIZE(AZ_CLASS_FROM_TYPE(aobj->array.collection.element_type)));
		if (az_value_equals_instance_autobox(AZ_IMPL_FROM_TYPE(aobj->array.collection.element_type), val, impl, inst)) return 1;
	}
	return 0;
}
...
array_object_get_element (const AZListImplementation *list_impl, void *list_inst, unsigned int idx, AZValue *val, unsigned int size)
{
	...
	return az_value_copy_autobox(AZ_IMPL_FROM_TYPE(aobj->array.collection.element_type), val, (const AZValue *) ((char *) aobj->array.values + idx * AZ_CLASS_ELEMENT_SIZE(AZ_CLASS_FROM_TYPE(aobj->array.collection.element_type))), size);
}
...
az_array_object_new(unsigned int elem_type, unsigned int length)
{
	...
	obj->array.collection.element_type = elem_type;
