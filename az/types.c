...
void
az_instance_initialize (void *inst, unsigned int type)
{
#ifdef AZ_SAFETY_CHECKS
...
az_interface_init (const AZImplementation *impl, void *inst)
{
	...
	arikkei_return_if_fail (AZ_TYPE_INDEX(AZ_IMPL_TYPE(impl)) < az_num_types);
...
implementation_init_recursive (AZInterfaceClass *iface_class, AZImplementation *impl)
{
	...
	if (klass->parent && (AZ_CLASS_TYPE(klass->parent) >= AZ_TYPE_INTERFACE)) {
