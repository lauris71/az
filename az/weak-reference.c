...
az_weak_reference_get_type (void)
{
	if (!weak_reference_type) {
		az_register_type (&weak_reference_type, (const unsigned char *) "WeakReference", AZ_TYPE_ANY, sizeof (AZWeakReferenceClass), sizeof (AZWeakReference), AZ_CLASS_ZERO_MEMORY | AZ_CLASS_IS_FINAL,
			NULL, NULL,
